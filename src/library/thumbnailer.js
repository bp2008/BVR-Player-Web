...
// Parameter sets sit at the front of a key frame; more than enough to configure.
const KEY_SAMPLE = 96 << 10
...
function paint (frame, header, maxW, maxH) {
...
export async function describeClip (blob, { maxWidth = THUMB_WIDTH, maxHeight = THUMB_HEIGHT } = {}) {
  ...
  try {
    ...
    if (keyBytes) {
      ...
      if (frame) {
        const painted = paint(frame, header, maxWidth, maxHeight)
        frame.close()
        if (painted) {
